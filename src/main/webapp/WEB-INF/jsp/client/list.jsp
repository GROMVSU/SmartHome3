      ...
      <TABLE>
        ...
        <c:forEach items="${applications}" var="application">
          ...
          <TR style="background: ${application.execution.identity == 7 ? 'red' : ''}">
            ...
            <TD>
            <c:choose>
            <c:when test="${application.execution.identity == 1}">
              <c:url value="/client/application/edit.html" var="applicationEditUrl"/>
              <FORM action="${applicationEditUrl}" method="post">
            ...
            <TD>
              ...
              <c:choose>
              <c:when test="${application.execution.identity == 7}"></c:when>
              <c:when test="${application.execution.identity == 1}">
              <c:url value="/client/application/delete.html" var="applicationDeleteUrl"/>
              <FORM action="${applicationDeleteUrl}" method="post">
                <INPUT type="hidden" name="identity" value="${application.identity}">
                <BUTTON type="submit">Удалить</BUTTON>
                    ...
                    <c:otherwise>
                      <c:url value="/client/application/withdraw.html" var="applicationWithdrawUrl"/>
                      <FORM action="${applicationWithdrawUrl}" method="post">
                        <INPUT type="hidden" name="identity" value="${application.identity}">
